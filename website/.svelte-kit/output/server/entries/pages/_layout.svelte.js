...
import { $ as attr, f as spread_props, g as html, h as unsubscribe_stores, l as head, mt as run, n as attr_class, o as derived, p as store_get } from "../../chunks/dev.js";
import { i as on } from "../../chunks/legacy-client.js";
import { t as page } from "../../chunks/stores.js";
import { n as MediaQuery, r as createSubscriber, t as Icon } from "../../chunks/Icon.js";
import "clsx";
...
function Mode_watcher_full($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		head("1funsus", $$renderer, ($$renderer) => {
			if (themeColors) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<meta name="theme-color"${attr("content", themeColors.dark)}/>`);
			} else $$renderer.push("<!--[-1-->");
...
function _layout($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		var $$store_subs;
		let { children } = $$props;
		const isDocs = derived(() => store_get($$store_subs ??= {}, "$page", page).url.pathname.startsWith("/docs"));
		Mode_watcher($$renderer, {});
		$$renderer.push(`<!----> <div class="bg-background min-h-screen"><header class="site-header"><div class="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14"><a href="/" class="font-semibold text-foreground text-lg tracking-tight flex items-center gap-2"><span class="inline-flex items-center justify-center w-7 h-7 rounded-md bg-[var(--accent)] text-[var(--accent-foreground)] text-xs font-bold">C</span> cora</a> <nav class="hidden sm:flex items-center gap-6"><a href="/docs"${attr_class("site-header-link", void 0, { "active": isDocs() })}>Docs</a> <a href="/docs/roadmap" class="site-header-link">Roadmap</a> <a href="https://github.com/codecoradev/cora-cli" target="_blank" rel="noopener" class="site-header-link">GitHub</a></nav> <div class="hidden sm:flex items-center gap-3"><button class="p-2 rounded-md text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors" aria-label="Toggle theme">`);
		Sun($$renderer, { class: "w-4 h-4 hidden dark:block" });
		$$renderer.push(`<!----> `);
		Moon($$renderer, { class: "w-4 h-4 block dark:hidden" });
		$$renderer.push(`<!----></button> <a href="/docs/getting-started" class="btn-primary text-[13px] px-3.5 py-1.5">Get Started</a></div> <div class="sm:hidden flex items-center gap-2"><button class="p-2 rounded-md text-[var(--muted-foreground)]" aria-label="Toggle theme">`);
		Sun($$renderer, { class: "w-4 h-4 hidden dark:block" });
		$$renderer.push(`<!----> `);
		Moon($$renderer, { class: "w-4 h-4 block dark:hidden" });
		$$renderer.push(`<!----></button> <button class="p-2 rounded-md text-[var(--muted-foreground)]" aria-label="Menu">`);
		$$renderer.push("<!--[-1-->");
